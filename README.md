...
**Condition A retry recovery.** When a first API call fails and a retry recovers, the retry output is written to `<study>_<model>_A_attempt2_raw.txt`. The raw-file parser in `score_results.py` reads only `<study>_<model>_A_raw.txt` (the first attempt), so a recovered retry file is not used in Condition A scoring. For this study all 14 first-attempt outputs were usable, so this did not affect the results.
